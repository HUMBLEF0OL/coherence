...
### 6b. Probe subagent attribution capability (FR-INSTALL-6)
- Check whether Claude Code can attribute tool calls to specific subagent invocations (line-level attribution).
- Attempt to read a recent metrics entry and verify whether `agent_id` fields are present at line granularity.
- Record: `subagent_attribution: "line-level" | "file-level-fallback" | "absent"`.
  - `line-level` — agent attribution available per-tool-call (preferred).
  - `file-level-fallback` — attribution only at file level (degraded, coherence can still operate).
  - `absent` — no subagent attribution; metrics per-agent stats will not be available.

### 7. Write results
- Save probe results to `.claude/coherence/host-capabilities.json` (atomic write):
```json
{
  "schema_version": "1.0",
...
## Constraints
- Read-only for user files: only writes to `.claude/coherence/host-capabilities.json`.
